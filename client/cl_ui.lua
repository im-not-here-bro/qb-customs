...
function InitiateMenus(isMotorcycle, vehicleHealth)
    ...
    for k, v in ipairs(vehicleCustomisation) do
        ...
        if amountValidMods > 0 or v.id == 18 then
            if v.id == 11 or v.id == 12 or v.id == 13 or v.id == 15 or v.id == 16 then --Performance Upgrades
                ...
                createMenu(v.category:gsub("%s+", "") .. "Menu", v.category, "Vælg en Opgradering")
            ...
            elseif v.id == 18 then
                ...
                createMenu(v.category:gsub("%s+", "") .. "Menu", v.category .. " Tilpasning", "Aktiver eller Deaktiver Turbo")

                populateMenu(v.category:gsub("%s+", "") .. "Menu", 0, "Deaktiver", "0 DKK")
                populateMenu(v.category:gsub("%s+", "") .. "Menu", 1, "Aktiver", "" .. vehicleCustomisationPrices.turbo.price .." DKK")

                updateItem2Text(v.category:gsub("%s+", "") .. "Menu", currentTurboState, "Installeret")

                finishPopulatingMenu(v.category:gsub("%s+", "") .. "Menu")
            else
                createMenu(v.category:gsub("%s+", "") .. "Menu", v.category .. " Tilpasning", "Vælg en Modifikation")
    ...
    --#[Respray Menu]#--
    createMenu("ResprayMenu", "Respray", "Vælg en Farve Kategori")
    ...
    populateMenu("ResprayMenu", 3, "Hjul Farve", "none")
    populateMenu("ResprayMenu", 4, "Indvendig Farve", "none")
    populateMenu("ResprayMenu", 5, "Dashboard Farve", "none")

    finishPopulatingMenu("ResprayMenu")

    --#[Respray Types]#--
    createMenu("ResprayTypeMenu", "Respray Typer", "Vælg en Farve Type")
    ...
    createMenu("WheelsMenu", "Hjul Kategorier", "Vælg en Kategori")
    ...
    for k, v in ipairs(vehicleWheelOptions) do
        if v.id == -1 then
            ...
            createMenu(v.category:gsub("%s+", "") .. "Menu", v.category, "Aktiver eller Deaktiver Tilpassede Hjul")

            populateMenu(v.category:gsub("%s+", "") .. "Menu", 0, "Deaktiver", "0 DKK")
            populateMenu(v.category:gsub("%s+", "") .. "Menu", 1, "Aktiver", "" .. vehicleCustomisationPrices.customwheels.price .." DKK")
        ...
        elseif v.id ~= 20 then
            if isMotorcycle then
                if v.id == 6 then --Motorcycle Wheels
                    ...
                    createMenu(v.category .. "Menu", v.category .. " Hjul", "Vælg et Hjul")
    ...
    createMenu("TyreSmokeMenu", "Tilpasning af dækrøg", "Vælg en Farve")
    ...
    createMenu("WindowTintMenu", "Tilpasning af vinduesfarve", "Choose a Tint")
    ...
    if livCount > 0 then
        ...
        createMenu("OldLiveryMenu", "Gammel Livery Tilpasning", "Vælg et Livery")
    ...
    createMenu("PlateIndexMenu", "Nummerplade Farve", "Vælg en Stil")
    local plateTypes = {
        "Blå på Hvid #1",
        "Gul på Sort",
        "Gul på Blå",
    ...
    --#[Vehicle Extras Menu]#--
    createMenu("VehicleExtrasMenu", "Køretøj Ekstras Tilpasning", "Ekstras Til/Fra")
    if GetVehicleClass(plyVeh) ~= 18 then
        for i=1, 12 do
    ...
    --#[Neons Menu]#--
    createMenu("NeonsMenu", "Neon Tilpasning", "Vælg en Kategori")
    ...
    for k, v in ipairs(vehicleNeonOptions.neonTypes) do
        ...
        createMenu(v.name:gsub("%s+", "") .. "Menu", "Neon Tilpasning", "Slå Neon Til/Fra")

        populateMenu(v.name:gsub("%s+", "") .. "Menu", 0, "Deaktiveret", "0 DKK")
        populateMenu(v.name:gsub("%s+", "") .. "Menu", 1, "Aktiveret", "" .. vehicleCustomisationPrices.neonside.price .." DKK")
    ...
    --#[Xenons Menu]#--
    createMenu("XenonsMenu", "Xenon Tilpasning", "Vælg en Kategori")
    ...
    createMenu("HeadlightsMenu", "Tilpasning af forlygter", "Slå Xenon Til/Fra")

    populateMenu("HeadlightsMenu", 0, "Deaktiver Xenons", "0 DKK")
    populateMenu("HeadlightsMenu", 1, "Aktiver Xenons", "" .. vehicleCustomisationPrices.headlights.price .." DKK")
